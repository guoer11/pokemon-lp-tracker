...
function compactAuth(){
  const auth=$('.auth');
  if(!auth||auth.dataset.v137==='1')return;
  auth.dataset.v137='1';
...
const leagueMap={
  '超級球':'league-great-v137',
  '紀念球':'league-premier-v137',
  '高級球':'league-ultra-v137',
  '大師球':'league-master-v137',
  '世界賽':'league-world-v137'
};

function decorateCard(card){
  if(!card?.classList?.contains('event-card'))return;
  const badges=$('.badges',card);if(!badges)return;
  const counted=$('.bestbadge',badges);
  if(counted&&counted.textContent.trim()!=='★ 計入8場')counted.textContent='★ 計入8場';
  [...badges.querySelectorAll('.badge')].forEach(b=>{
    const text=b.textContent.trim();
    for(const [name,cls] of Object.entries(leagueMap))if(text===name){b.classList.add(cls);break}
  });
  const meta=$('.meta',card),m=meta?.textContent.match(/第\s*(\d+)\s*名/),place=m?Number(m[1]):0;
  let top=$('.top8badge-v137',badges);
  if(place>=1&&place<=8){
    if(!top){top=document.createElement('span');top.className='badge top8badge-v137';top.textContent='TOP 8';badges.insertBefore(top,badges.firstChild)}
