...
async function handle(ev){
 const form=ev.currentTarget,submit=$('submitBtn');
 ...
 if(!session)return; // 未登入時交給原本本機儲存流程
 ...
 const id=(crypto.randomUUID&&crypto.randomUUID())||('evt-'+Date.now()+'-'+Math.random().toString(16).slice(2));
